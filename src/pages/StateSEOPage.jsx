...
import { stateSlugToInfo } from '../data/locationSlugs.js'
import { buildStatePageData } from '../data/generateLocationContent.js'
import { useNoIndex } from '../hooks/useNoIndex.js'

function StateSEOPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <div className="text-center">
          ...
          <p className="mt-3 text-slate-600">Pharmacy software page for this state is not yet available.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <article className="bg-white">
      <Helmet>
        ...
        <link rel="canonical" href={`https://www.aadhiraiinnovations.com/pharmacy-billing-software/state/${stateSlug}`} />
        <meta property="og:title" content={data.meta.title} />
        <meta property="og:description" content={data.meta.description} />
        <meta property="og:type" content="website" />
        <meta property="og:url" content={`https://www.aadhiraiinnovations.com/pharmacy-billing-software/state/${stateSlug}`} />
        <meta property="og:site_name" content="Aadhirai Innovations" />
        <script type="application/ld+json">
          {JSON.stringify({
            '@context': 'https://schema.org',
            '@type': 'LocalBusiness',
            name: `Medora+ - Pharmacy Billing Software in ${state}`,
            description: data.meta.description,
            url: `https://www.aadhiraiinnovations.com/pharmacy-billing-software/state/${stateSlug}`,
        ...
        <Container>
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {districts.map((d) => (
              <Link
                key={d.slug}
                to={`/pharmacy-billing-software/${d.slug}`}
