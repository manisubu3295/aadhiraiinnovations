...
import { cityPageData } from '../components/sections/HrmLocationPageTemplate'
import { districtSlugToInfo, CURATED_ALIASES } from '../data/hrmLocationSlugs.js'
import { buildDistrictPageData } from '../data/generateHrmLocationContent.js'
import { useNoIndex } from '../hooks/useNoIndex.js'

function HrmLocalSEOPage() {
  const { city } = useParams()
  const citySlug = city?.toLowerCase()

  // 1. A hand-written curated page (HrmLocationPageTemplate.jsx) always wins.
  // 2. A few dataset districts are known by a different official name than their curated page
  //    (e.g. Bengaluru's "Urban" district) — redirect rather than 404 or render a competing
  //    near-duplicate page for the same real place.
  // 3. Otherwise, generate content on the fly for any of the ~714 non-curated Indian districts.
  ...
  if (!data) {
    if (isAlias) {
      return <Navigate to={`/hrm-software/${CURATED_ALIASES[citySlug]}`} replace />
    }
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-[#0B1F3A]">Page Not Found</h1>
          <p className="mt-3 text-slate-600">HRM software page for {city} is not yet available.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <article className="bg-white">
      ...
      <Helmet>
        ...
        <link rel="canonical" href={`https://www.aadhiraiinnovations.com/hrm-software/${citySlug}`} />

        {/* Open Graph tags for social sharing */}
        <meta property="og:title" content={data.meta.title} />
        <meta property="og:description" content={data.meta.description} />
        <meta property="og:type" content="website" />
        <meta property="og:url" content={`https://www.aadhiraiinnovations.com/hrm-software/${citySlug}`} />
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "@type": "LocalBusiness",
            "name": `HR & Inventory - HRM Software in ${data.city}`,
            "description": data.meta.description,
            "url": `https://www.aadhiraiinnovations.com/hrm-software/${citySlug}`,
            "areaServed": {
              "@type": "City",
              "name": data.city,
...
export default HrmLocalSEOPage
