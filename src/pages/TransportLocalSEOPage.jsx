...
import { cityPageData } from '../components/sections/TransportLocationPageTemplate'
import { districtSlugToInfo, CURATED_ALIASES } from '../data/transportLocationSlugs.js'
import { buildDistrictPageData } from '../data/generateTransportLocationContent.js'
import { useNoIndex } from '../hooks/useNoIndex.js'

function TransportLocalSEOPage() {
  const { city } = useParams()
  const citySlug = city?.toLowerCase()

  // 1. A hand-written curated page (TransportLocationPageTemplate.jsx) always wins.
  // 2. A few dataset districts are known by a different official name than their curated page
  //    (e.g. Bengaluru's "Urban" district, Delhi's "New Delhi" borough) — redirect rather than
  //    404 or render a competing near-duplicate page for the same real place.
  ...
  if (!data) {
    if (isAlias) {
      return <Navigate to={`/transport-software/${CURATED_ALIASES[citySlug]}`} replace />
    }
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-[#0B1F3A]">Page Not Found</h1>
          <p className="mt-3 text-slate-600">Transport software page for {city} is not yet available.</p>
        </div>
      </div>
    )
  }
...
export default TransportLocalSEOPage
