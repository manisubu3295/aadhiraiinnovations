...
import { lessonData } from '../data/lessons/strings'

export default function JavaDsaStringsPage() {
  const navMap = {
  ...
  const { prev, next } = navMap['strings']
  return (
    <InteractiveLessonPage
      lessonData={lessonData}
      difficulty={['big-o','strings','hash-maps'].includes('strings') ? 'Foundations' : ['bubble-sort','merge-sort'].includes('strings') ? 'Intermediate' : 'Advanced'}
      prev={prev}
      next={next}
    />
  )
