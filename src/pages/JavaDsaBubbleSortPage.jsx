...
import { lessonData } from '../data/lessons/bubble-sort'

export default function JavaDsaBubbleSortPage() {
  const navMap = {
  ...
  const { prev, next } = navMap['bubble-sort']
  return (
    <InteractiveLessonPage
      lessonData={lessonData}
      difficulty={['big-o','strings','hash-maps'].includes('bubble-sort') ? 'Foundations' : ['bubble-sort','merge-sort'].includes('bubble-sort') ? 'Intermediate' : 'Advanced'}
      prev={prev}
      next={next}
    />
  )
