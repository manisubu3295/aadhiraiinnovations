...
import { states } from '../data/locationSlugs.js'

function PharmacyLocationsHubPage() {
  const totalDistricts = states.reduce((sum, s) => sum + s.districts.length, 0)

  return (
    <article className="bg-white">
      <Helmet>
        <title>Pharmacy Billing Software Across India — All States &amp; Districts | Aadhirai Innovations</title>
        <meta
          name="description"
          content={`Medora+ pharmacy billing software availability across ${states.length} Indian states/UTs and ${totalDistricts} districts. GST-ready, offline-first, built for Indian pharmacies.`}
        />
        <link rel="canonical" href="https://aadhiraiinnovations.com/pharmacy-billing-software" />
      </Helmet>
        ...
        <Container className="relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="mb-4 text-4xl font-bold tracking-[-0.03em] text-[#0B1F3A] sm:text-5xl leading-[1.1]">
              Pharmacy Billing Software Across India
            </h1>
            <p className="mb-6 max-w-2xl text-xl text-slate-600">
              Medora+ availability across {states.length} states/UTs and {totalDistricts} districts.
            </p>
            <p className="max-w-2xl text-[16px] text-slate-700 leading-[1.8]">
              GST-ready billing, offline-first design, and real-time inventory — built for Indian pharmacies,
              wherever your medical shop is located. Browse your state below to find pharmacy software
              details for your specific district.
            </p>
          </motion.div>
        </Container>
      </section>
        ...
        <Container>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {states.map((s, i) => (
              ...
              >
                <Link
                  to={`/pharmacy-billing-software/state/${s.stateSlug}`}
...
export default PharmacyLocationsHubPage
