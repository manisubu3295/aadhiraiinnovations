...
import { arraysSteps } from '../data/arraysSteps'
...
export default function StepEngine({ onStepChange, onExerciseComplete }) {
  ...
  const currentStep = arraysSteps[stepIndex]
  const isLastStep = stepIndex === arraysSteps.length - 1
  ...
  return (
    <div className="w-full space-y-8">
      ...
      <div className="flex justify-between items-center text-sm text-slate-500 mt-12">
        ...
        <span className="font-medium">{stepIndex + 1} of {arraysSteps.length}</span>
        <div />
      </div>
    </div>
  )
